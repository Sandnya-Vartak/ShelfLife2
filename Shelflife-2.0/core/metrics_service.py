...
def get_consumption_summary(user_id):
    ...
    wasted_notifications = [notif for notif in notifications if notif.status == "wasted"]
    wasted_alert_source = wasted_notifications or read_not_consumed
        ...
        "wasted_alerts": [
            {
                ...
                "currency": (notif.item.currency if notif.item else "USD") or "USD",
                "wasted_amount": to_float(item_total_cost(notif.item)) if notif.item else 0,
            }
            for notif in wasted_alert_source[:10]
        ],
    }
