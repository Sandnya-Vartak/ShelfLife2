...
def _set_last_mail_error(app, message=""):
    app.extensions["shelflife_last_mail_error"] = message or ""


def get_last_mail_error():
    try:
        return current_app.extensions.get("shelflife_last_mail_error", "")
    except Exception:
        return ""
...
def _send_with_sendgrid(app, to, subject, body):
    ...
    if 200 <= response.status_code < 300:
        _set_last_mail_error(app, "")
        print(f"[ok] Email sent to {to} via SendGrid: {subject}")
        return True
    _set_last_mail_error(app, f"SendGrid failed with status {response.status_code}")
...
def _send_with_smtp(app, to, subject, body):
    if not _has_real_mail_credentials(app):
        _set_last_mail_error(app, "SMTP credentials are missing or MAIL_USERNAME is not a real email address.")
        print("[warn] SMTP credentials not configured. Set MAIL_USERNAME to a real email address and MAIL_PASSWORD in .env.")
        return False
...
def send_email(to, subject, body):
    ...
    try:
        ...
        if not to or "@" not in str(to):
            _set_last_mail_error(app, f"Invalid recipient email address: {to}")
            print(f"[warn] Invalid email address: {to}")
            return False

        if app.config.get("MAIL_TEST_MODE"):
            _set_last_mail_error(app, "MAIL_TEST_MODE is enabled, so real emails are skipped.")
        ...
        if provider == "sendgrid":
            if not _has_real_sendgrid_credentials(app):
                _set_last_mail_error(app, "SendGrid is selected but SENDGRID_API_KEY or SENDGRID_FROM_EMAIL is missing.")
        ...
        _set_last_mail_error(app, str(error))
...
def send_expiry_alert_email(to, message, status):
    try:
        subject_map = {
            ...
            "wasted": "ShelfLife Alert: Money Wasted",
